-- The Band 4.0 content pipeline's fallback example generator produced only
-- 2 sentence templates per part of speech (e.g. "The {word} is important
-- for the campus project."), used for any word lacking a real authored
-- example. Words that don't fit the template shape grammatically (seasons,
-- gerunds, adjectives used as nouns, ...) produced nonsense: "The autumn is
-- important for the campus project.", "The dining is important for the
-- campus project.", "The double answer was easy to understand."
--
-- Rather than delete those examples rows (the static Band 4.0 CSV import
-- references some of them via questions.example_id, so deleting risks a
-- foreign key violation), make generate_practice_question() treat them as
-- if no example existed: skip any example whose sentence_en is an exact
-- reconstruction of one of the known fallback templates. When no example
-- survives, sentence_cloze_typing now asks the learner to spell the word
-- directly from its Chinese meaning instead of showing a mismatched
-- sentence.
...
begin
  ...
  if p_question_type_key in ('sentence_cloze_typing', 'reading_comprehension') then
    ...
    where e.sense_id = p_sense_id
      ...
      and e.sentence_en <> all(array[
        'The ' || e.target_span || ' is important for the campus project.',
        'Please check the ' || e.target_span || ' before class starts.',
        'Students need to ' || e.target_span || ' their ideas clearly.',
        'I will ' || e.target_span || ' the answer after the lecture.',
        'The ' || e.target_span || ' answer was easy to understand.',
        'Her tutor gave a ' || e.target_span || ' example in class.',
        'She answered the question ' || e.target_span || ' during the tutorial.',
        'He explained the problem ' || e.target_span || ' to his group.',
        'The professor used ' || e.target_span || ' in a simple sentence.',
        'I noticed ' || e.target_span || ' in the reading passage.'
      ])
  ...
  v_stem := case p_question_type_key
    ...
    when 'sentence_cloze_typing' then
      case when v_example.id is not null then
        ...
        'Spell out the English word for: ' || coalesce(nullif(v_definition_zh, ''), v_definition_en) || '.'
      end
...
-- Backfill already-generated questions whose stem baked in one of the
-- mismatched fallback templates (identifiable once blanked, since the
-- blank replaces the only place the headword appeared).
update public.questions q
set stem = 'Spell out the English word for: ' || coalesce(nullif(ws.definition_zh, ''), ws.definition_en) || '.'
from public.word_senses ws
where q.question_type_key = 'sentence_cloze_typing'
  and ws.id = q.sense_id
  and q.stem = any(array[
    'The ___ is important for the campus project.',
    'Please check the ___ before class starts.',
    'Students need to ___ their ideas clearly.',
    'I will ___ the answer after the lecture.',
    'The ___ answer was easy to understand.',
    'Her tutor gave a ___ example in class.',
    'She answered the question ___ during the tutorial.',
    'He explained the problem ___ to his group.',
    'The professor used ___ in a simple sentence.',
    'I noticed ___ in the reading passage.'
  ]);

update public.questions q
set stem = 'Choose the word that best fits this context: This word means "' || ws.definition_en || '".'
from public.word_senses ws
where q.question_type_key = 'reading_comprehension'
  and ws.id = q.sense_id
  and q.stem = any(array[
    'Choose the word that completes this sentence: The ___ is important for the campus project.',
    'Choose the word that completes this sentence: Please check the ___ before class starts.',
    'Choose the word that completes this sentence: Students need to ___ their ideas clearly.',
    'Choose the word that completes this sentence: I will ___ the answer after the lecture.',
    'Choose the word that completes this sentence: The ___ answer was easy to understand.',
    'Choose the word that completes this sentence: Her tutor gave a ___ example in class.',
    'Choose the word that completes this sentence: She answered the question ___ during the tutorial.',
    'Choose the word that completes this sentence: He explained the problem ___ to his group.',
    'Choose the word that completes this sentence: The professor used ___ in a simple sentence.',
    'Choose the word that completes this sentence: I noticed ___ in the reading passage.'
  ]);
