...
> **Progression-model correction (2026-06-24):** Any initial assessment,
> `assessment_results`, or calculated starting-level design in this historical
> construction brief is superseded by
> `../plans/BAND_UPGRADE_EXAM_PLAN.md`. Content must support 40-question
> upgrade exams grouped by internal `curriculum_band` and learner-facing
> `雅思 # 分难度`.

> **Beginner-definition rule (2026-06-24):** In Levels 1–5, recognition
> questions must display the reviewed Chinese sense definition. Do not require
> a learner to understand an English dictionary gloss that may contain words
> harder than the target. English definitions remain stored for later
> reading/review activities. Every selected sense must match the examples and
> Chinese gloss; do not merge unrelated dictionary senses into one option.
...
> Cross-references to `../architecture/DATA_MODEL_AND_CAPACITY.md`,
> `../architecture/CONTENT_DATA_SOURCE_POLICY.md`, and
...
1. Source vocabulary words from approved references (see §6 and `../architecture/CONTENT_DATA_SOURCE_POLICY.md`)
...
data, with no fabricated content, for at least one full level (approximately
80 learning slots, not 80 entirely new words).**
...
-- EVERY sentence must come from an approved source in CONTENT_DATA_SOURCE_POLICY.md §3.
...
All approved sources are listed in `../architecture/CONTENT_DATA_SOURCE_POLICY.md`.
Read it before producing any data. Key rules:

| Data type | Approved source |
|-----------|----------------|
| Word list / level assignment | Cambridge IELTS list, Oxford 3000/5000 (see `../architecture/CONTENT_DATA_SOURCE_POLICY.md §1`) |
...
| Phase | Scope | Goal |
|-------|-------|------|
| **Pilot** | Level 1 only (45 new senses plus form/collocation and review slots, IELTS 4.0) | Validate end-to-end: word packages load → questions generate → app renders them → grading works |
...
| Level | One of 240 game levels, each containing approximately 80 configurable learning slots rather than 80 entirely new words. Levels map to IELTS bands (4.0–8.0). |
...
*For approved source URLs and citation requirements, see `../architecture/CONTENT_DATA_SOURCE_POLICY.md`.*  
*For app architecture and repository interfaces, see `../architecture/APP_ARCHITECTURE.md`.*  
*For full table schemas and volume projections, see `../architecture/DATA_MODEL_AND_CAPACITY.md`.*
